...
#IRDIR = "F:\IRimages2016"
IRDIR = "K:\IRimages2017"
...
if BASIN == "WP":
    ...
    LAT_BOUND = [-20,60] #WP Basin
    LON_BOUND = [160,300] #WP Basin
    B_tracks = xr.open_dataset(BTDIR+"\\"+"IBTrACS.EP.v04r00.nc")
elif BASIN == "SP": 
    LAT_BOUND = [-60,20] #WP Basin
    LON_BOUND = [120,260] #WP Basin
    B_tracks = xr.open_dataset(BTDIR+"\\"+"IBTrACS.SP.v04r00.nc")
elif BASIN == "NA": 
    LAT_BOUND = [-20,60] #WP Basin
    LON_BOUND = [240,360] #WP Basin
    B_tracks = xr.open_dataset(BTDIR+"\\"+"IBTrACS.NA.v04r00.nc")
elif BASIN == "SI": 
    LAT_BOUND = [-60,20] #WP Basin
...
#%
BTempimage = xr.open_dataset(r"K:\IRimages2016\REFFILE_360\merg_2016010100_4km-pixel.nc4")
...
C_lat = BTemp_lat[DIM_BOUND[0]:DIM_BOUND[1]+1]
C_lon = BTemp_lon[DIM_BOUND[2]:DIM_BOUND[3]+1]
...
I_lon_180 = I_time_interpolate['lon']
I_lon = np.where(I_lon_180<0,I_lon_180+360,I_lon_180)
#%
SAVDIR = r"K:\THEJUDGEMENT\BASINS_RESULTS_" + CHOSEN_YEAR + "\\"+ BASIN + "\\" + TC_serial + "_" + I_name
os.mkdir(SAVDIR)
...
##%
    ...
    plt.show()
    #%% Start spreading
    BOUNDADRY_R = 550#Default: 500
    NO_MAXIMA = 50 #Default: 16
    C_BTEMP_MAX = 260 #Default: 270
...
#    C_flag = np.where(C_binary8_filled8 == 1,2,0)
#    C_flag = np.where(C_binary8 > 0,2,0)
...
#    C_flag = np.where(blobs_labels == 6,2,C_flag)
#    C_flag = np.where(blobs_labels == 7,2,C_flag)
#    C_flag = np.where(blobs_labels == 8,2,C_flag)
#    C_flag = np.where(blobs_labels == 9,2,C_flag)
#    C_flag = np.where(blobs_labels == 10,2,C_flag)
#    C_flag = np.where(blobs_labels == 11,2,C_flag)
#    C_flag = np.where(blobs_labels == 12,2,C_flag)
#    C_flag = np.where(blobs_labels == 13,2,C_flag)
...
#    im = plt.imshow(C_binary8, extent = (lon_min, lon_max, lat_min, lat_max),  cmap='Greys_r',origin='lower')
    im = plt.imshow(dist_transform_second, extent = (lon_min, lon_max, lat_min, lat_max),  cmap=plt.cm.nipy_spectral,origin='lower')
    # Best track center
    plt.plot(I_lon[C_i],I_lat[C_i],'or', markersize = 2) 
