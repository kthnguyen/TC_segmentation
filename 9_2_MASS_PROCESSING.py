...
#    BTemp_lon = BTempimage['lon'].values[:]
    ...
    lon_bound = [i for i,val in enumerate(BTemp_lon_360) if (val>lonmin and val<lonmax)]   
    lon_val_bound = [val for i,val in enumerate(BTemp_lon_360) if (val>lonmin and val<lonmax)] 
...
#%%
a = np.array(range(9,22))
b = np.array(range(24,33))
c = np.append(a,b)
for i in range(0 ,14):
    ...
    I_lon_180 = pd.DataFrame(I_lon).dropna().values[:,0]
    ...
    ##%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    #% Find out first step to start with (as first non all zero label matrix in C_label_TC)
    C_i_start = 1
    C_i_check_flag = 0
    while C_i_check_flag == 0:
        C_flag_check = C_label_TC[C_i_start-1,:,:]
        if C_flag_check.nonzero()[1].shape[0]>0:
            C_i_check_flag = 1
        else:
            C_i_start = C_i_start + 1
...
#        C_flag_temp = C_flag_prev
        ...
        C_flag_core = np.where(C_BTemp > C_BTEMP_MAX, 0,C_flag_temp)
...
#        for label in range(135,136):
    ...
    #%de')
        ...
        fig.savefig(SAVDIR + "\\" + filename +".png",dpi=72)
        plt.close()
